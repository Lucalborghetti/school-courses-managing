  ...
  export default {
    ...
    created() {
      if (!this.user_role) {
        ...
        axios.get('https://localhost:8443/courses', {withCredentials: true})
          .then(response => {
    ...
    methods: {
      postAdd(event) {
        axios.post('https://localhost:8443/courses/', {
          titre: this.titre,
      ...
      duplicateCourse(event) {
        axios.get(`https://localhost:8443/courses/${this.duplicatedCours.coursId}/implication`, {withCredentials: true})
          .then(response => {
