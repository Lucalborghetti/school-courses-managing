    ...
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
      <div class="container-fluid">
        <div class="navbar-header" v-if="this.$global.user_role">
          <a class="navbar-brand" href="#/home">Gestion des chapitres choisis</a>
        </div>
        <div class="navbar-header" v-else>
          <a class="navbar-brand">Gestion des chapitres choisis</a>
        </div>
        <ul class="nav navbar-nav navbar-right" v-if="this.$global.user_role">
          <li><a class="padding-right">Signed in as {{ this.$global.user_firstname }} {{ this.$global.user_lastname
            }}</a></li>
          ...
          buttons: [
            {
              title: 'Logout',
              handler: () => {
                axios.get('https://localhost:8443/logout', {withCredentials: true})
                  .then(response => {
